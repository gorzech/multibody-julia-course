...
# ╔═╡ 05c0a9a5-17ef-416f-809f-8c3ca43c0b77
### # Pluto Notebook: Lagrange EoM for Block‐Rod System
begin
    ### Load necessary packages
    using Symbolics
	import Symbolics: derivative, expand_derivatives, simplify, substitute, build_function
    using LinearAlgebra
	using DifferentialEquations
end

# ╔═╡ a477c7b9-d0d9-49b5-b78a-8f0da657030c
using Plots

# ╔═╡ 26f3448a-e62e-4d22-9c29-f850fef382d8
begin
    ### Define independent variable and generalized coordinates
    @variables t
    @variables x(t) θ(t)
    D = Differential(t)
end

# ╔═╡ c0b927a3-73f4-450e-8326-9e7acc6909be
### Define system parameters
@variables m L k c

# ╔═╡ 6724bd06-a775-41a3-9ff4-3a14cebc4394
begin
    ### Compute time‐derivatives (velocities)
    dx = D(x)
    dθ = D(θ)
end

# ╔═╡ cb57d082-6505-4d4e-8f0a-8ed6116af41b
begin
    ### Kinetic energy T
    # Block: 1/2*m*dx^2
    T_blk = 1//2*m*dx^2
    # Rod: mass 2m, CM at L/2; translational + rotational
    v_cm_sq = (dx + (L/2)*cos(θ)*dθ)^2 + ((L/2)*sin(θ)*dθ)^2
    I_cm = (2m)*L^2/12
    T_rod = 1//2*(2m)*v_cm_sq + 1//2*I_cm*dθ^2
    T = simplify(expand(T_blk + T_rod))
end

# 6

# ╔═╡ 0da6529a-4062-4441-8e94-0820714b3770
begin
    ### Potential energy V
    # Spring on block: 1/2*k*x^2
    V_blk = 1//2*k*x^2
    # Bottom spring (2k) at tip: horizontal pos = x + L*sin(θ)
    V_tip = 1//2*(2k)*(x + L*sin(θ))^2
    V = expand(V_blk + V_tip)
end

# ╔═╡ d31df7fb-abca-4a69-b6df-8692691cb78c
### Rayleigh dissipation D_fun
# Damper on block (c*dx) and at tip (c*(dx + L*cos θ * dθ))
D_fun = 1//2*c*dx^2 + 1//2*c*(dx + L*cos(θ)*dθ)^2

# ╔═╡ 52106005-09dd-48be-9c5a-8cd5cdaae9cf
### Lagrangian L = T - V
Lagr = T - V

# ╔═╡ 6d30eeb7-3a8e-4dc2-8f7b-7f656123eab4
begin
    ### Euler–Lagrange equations with dissipation
    ∂L_∂dx = derivative(Lagr, dx)
    ∂L_∂x  = derivative(Lagr, x)
    ∂D_∂dx = derivative(D_fun, dx)
    eq_x = expand_derivatives(D(∂L_∂dx) - ∂L_∂x + ∂D_∂dx)

    ∂L_∂dθ = derivative(Lagr, dθ)
    ∂L_∂θ  = derivative(Lagr, θ)
    ∂D_∂dθ = derivative(D_fun, dθ)
    eq_θ = expand_derivatives(D(∂L_∂dθ) - ∂L_∂θ + ∂D_∂dθ)

    Eqs = simplify.([eq_x ~ 0, eq_θ ~ 0])
end

# ╔═╡ 40319f59-1f1b-4e9f-be5c-8757470c8b8a


# ╔═╡ 5e4cb175-e8b6-42a9-9a57-0b279c5c70c3
begin
    ### Manually solve for accelerations without ModelingToolkit
    # Introduce symbols for accelerations
    @variables ddx(t) ddθ(t)
    # Replace second derivatives in equations
    eqs_repl = substitute(Eqs, Dict(D(D(x)) => ddx, D(D(θ)) => ddθ))
    # Solve linear system for ddx and ddθ
    sol_acc = symbolic_linear_solve(eqs_repl, [ddx, ddθ])
	# Extract explicit expressions
    expr_ddx = sol_acc[1]
    expr_ddθ = sol_acc[2]
end

# ╔═╡ 67ebc999-5a58-40f4-8a73-d92758806325
f_acc = build_function(sol_acc, [x, θ, dx, dθ, m, L, k, c], expression=Val{false})[1]

# ╔═╡ e811fc5e-2ccd-420d-9bc1-28778c3a3519
f_acc([0.1, 0.01, 1.0, 0.2, 1, 1, 100, 0.1])

# ╔═╡ 976aa75b-ae32-4a9a-8fda-151af08982f1


# ╔═╡ 7b854851-1628-4162-9ebf-1d7fde787dc4
md"""
## Numerical ODE via DifferentialEquations.jl without ModelingToolkit

Define a standard function `f(u,p,t)` using the derived accelerations:

"""

# ╔═╡ 5b1c6a59-02f5-44f1-a362-18332ef4ba71
# State vector: u = [x, θ, vx, vθ]
f(u, p, t) = begin
    x, θ, vx, vθ = u
    m, L, k, c = p
    # from symbolic solution:
	ddx, ddθ = f_acc([x, θ, vx, vθ, m, L, k, c])
    return [vx, vθ, ddx, ddθ]
end

# ╔═╡ 95303bc3-bbf6-4170-a59d-f34d5ed9ed34
u0 = [0.0, 0.1, 0.0, 0.0]

# ╔═╡ a586c175-2e0e-4a78-a771-9e4d0170908d
p_val = (1.0, 1.0, 10.0, 0.5)

# ╔═╡ 48e3ebf0-66be-47fd-abf1-eb1f7d5deafc
prob = ODEProblem(f, u0, (0.0, 10.0), p_val)

# ╔═╡ 453828be-9e0b-4a4e-b1b6-ff3bbd447183
sol_num = solve(prob, Tsit5())

# ╔═╡ e6660a06-9ef3-4c30-9916-68e0f03d6137
plot(sol_num, idxs=[(0, 1),(0, 2)], label=["x(t)" "θ(t)"], xlabel="t", ylabel="states")

# ╔═╡ 6eb32386-8bac-4010-84d8-787d485154db
md"""
## Energies

**Kinetic Energy**

$$T = \frac32 m \dot x^2 + m L \cos\theta\,\dot x\,\dot\theta + \frac{mL^2}{3}\dot\theta^2$$

**Potential Energy**

$$V = \frac12 k x^2 + k(x + L\sin\theta)^2$$

**Dissipation**

$$D = \frac12 c \dot x^2 + \frac12 c(\dot x + L\cos\theta\,\dot\theta)^2$$
"""

# 12

# ╔═╡ b092d4a8-1886-43f3-a1b7-f03f88f7a7b1
md"""
## Equations of Motion

$$\frac32 m \ddot x + mL\cos\theta\,\ddot\theta - mL\sin\theta\,\dot\theta^2 + 2c\dot x + cL\cos\theta\,\dot\theta + 3k x + 2kL\sin\theta = 0$$

$$mL\cos\theta\,\ddot x + \frac{2}{3}mL^2\ddot\theta + cL\cos\theta\,\dot x + cL^2\cos^2\theta\,\dot\theta + 2kL\cos\theta\,(x+L\sin\theta) = 0$$
"""

# ╔═╡ b024733c-a1bf-42f1-9258-19139f41f9a8
md"""
...
manifest_format = "2.0"
project_hash = "a929b42c7fa730b7d661ad4d292e2129f89cd77a"
