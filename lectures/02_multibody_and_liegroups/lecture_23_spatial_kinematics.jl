...
## 3D Rigid Body Pose (Position + Orientation Quaternion)

  

In the spatial (3D) formulation, each rigid body’s **pose** is described by a position vector $\boldsymbol{r}_i \in \mathbb{R}^3$ and an orientation quaternion $\boldsymbol{q}_i \in \mathbb{R}^4$. The quaternion $\boldsymbol{q}_i = [e{0}, e{1}, e_{2}, e_{3}]^\top$ (also known as **Euler parameters**) is a unit vector ($|\boldsymbol{q}_i|=1$) representing the body’s rotation in 3D. 

The position $\boldsymbol{r}_i = [x_i,, y_i,, z_i]^\top$ locates the body’s reference point in the global frame, while the unit quaternion $\boldsymbol{q}_i$ defines the body’s orientation without singularities. (In planar dynamics, orientation was given by a single angle $\phi_i$; here we use four Euler parameters to capture 3D orientation.)

- **Position**: $\boldsymbol{r}_i = (x_i,, y_i,, z_i)^\top$ describes the location of body $i$ in the global frame.
    
- **Orientation**: $\boldsymbol{q}_i = (e_{0}, e_{1}, e_{2}, e_{3})^\top$ is a unit quaternion (Euler parameters) representing the rotation from body $i$’s local frame to the global frame. The quaternion relates to a rotation angle $\theta$ about a unit axis $\mathbf{u}=(u_x,u_y,u_z)$ by $e_{0}=\cos(\theta/2)$, $;e_{1}=u_x\sin(\theta/2)$, etc.
...
## Rotation Matrix from Quaternion (Euler Parameters)

Each quaternion $\boldsymbol{q}_i$ corresponds to a $3\times 3$ rotation matrix $\boldsymbol{A}(\boldsymbol{q}i)$ that transforms vectors from the body’s local coordinate frame to the global frame. If $\boldsymbol{q}i = [e{0}, e{1}, e_{2}, e_{3}]^\top$, the rotation matrix $\boldsymbol{A}(\boldsymbol{q}_i)$ (also called a direction cosine matrix) can be computed as :

$\boldsymbol{A}(\boldsymbol{q}_i) \;=\; \begin{bmatrix} e_{0}^2 + e_{1}^2 - e_{2}^2 - e_{3}^2 & \;2(e_{1}e_{2} - e_{0}e_{3}) & \;2(e_{1}e_{3} + e_{0}e_{2})\\[6pt] 2(e_{1}e_{2} + e_{0}e_{3}) & \;e_{0}^2 - e_{1}^2 + e_{2}^2 - e_{3}^2 & \;2(e_{2}e_{3} - e_{0}e_{1})\\[6pt] 2(e_{1}e_{3} - e_{0}e_{2}) & \;2(e_{2}e_{3} + e_{0}e_{1}) & \;e_{0}^2 - e_{1}^2 - e_{2}^2 + e_{3}^2 \end{bmatrix},$

  

which is a proper orthonormal rotation matrix ($\boldsymbol{A}^\top \boldsymbol{A} = \boldsymbol{I}_{3}$ and $\det \boldsymbol{A}=+1$). This matrix reduces to the planar rotation matrix $A_i(\phi_i)$ when rotations are about a single axis. In 3D, however, $\boldsymbol{A}(\boldsymbol{q}_i)$ can represent an arbitrary orientation in space as specified by the quaternion.
...
## Point Position Transformation (Local to Global Frame)

To find the global coordinates of a point $S$ that is fixed on body $i$, we transform the point’s local coordinates to the world frame using $\boldsymbol{r}_i$ and $\boldsymbol{A}(\boldsymbol{q}_i)$. If $\boldsymbol{s}_i^S$ is the position of point S in **body $i$’s local frame** (expressed as a constant vector in body $i$ coordinates), then the **global position** $\boldsymbol{r}^S$ of that point is given by:
...
## Example: Computing Global Position of a Body-Fixed Point

**Given:** A rigid body with position $\boldsymbol{r}_i = [\,1\,;2\,;3\,]^\top$ and orientation $\boldsymbol{q}_i = [\cos 45^\circ\,;0\,;\sin 45^\circ\,;0]^\top = [0.7071\,;0\,;0.7071\,;0]^\top$. This quaternion represents a $90^\circ$ rotation about the global $y$-axis. 


Consider a point S fixed in this body with local coordinates $\boldsymbol{s}_i^S = [\,1\,;0\,;0\,]^\top$ (1 unit along the body’s local $x$-axis).

- **Rotation Matrix:** First, compute $\boldsymbol{A}(\boldsymbol{q}_i)$ for $\boldsymbol{q}_i = [0.7071\,;0\,;0.7071\,;0]^\top$. Plugging into the formula, we get:
    
$\boldsymbol{A}(\boldsymbol{q}_i) \;=\; \begin{bmatrix} 0 & 0 & 1\\[3pt] 0 & 1 & 0\\[3pt] -1 & 0 & 0 \end{bmatrix},$

which indeed corresponds to a $90^\circ$ rotation about $y$ (the body’s local $x$-axis is rotated to align with the global $-z$ direction, etc.).
    
- **Global Position:** Now apply the transformation:
    
$\boldsymbol{r}^S \;=\; \boldsymbol{r}_i + \boldsymbol{A}(\boldsymbol{q}_i)\,\boldsymbol{s}_i^S \;=\; \begin{bmatrix}1\\2\\3\end{bmatrix} + \begin{bmatrix}0 & 0 & 1\\ 0 & 1 & 0\\ -1 & 0 & 0\end{bmatrix} \begin{bmatrix}1\\0\\0\end{bmatrix} \;=\; \begin{bmatrix}1\\2\\2\end{bmatrix}~.$
    
Thus, point S’s global coordinates are $\boldsymbol{r}^S = [\,1\,;2\,;2\,]^\top$. The procedure is: (1) rotate the local point by $\boldsymbol{A}(\boldsymbol{q}_i)$, (2) then add the body’s global position $\boldsymbol{r}_i$. This example demonstrates the spatial formula for a specific quaternion and point.
    
"""

# ╔═╡ 7be3784c-0c84-4e3e-ba6a-c80b17122ee5
md"""
## Relative Constraints Between Two Vectors

Many kinematic constraints in multibody systems involve vectors that must remain **parallel**, **perpendicular**, or have a **fixed length**.

Vectors may:
- Be fixed within the same body
- Connect points on different bodies

All vectors must be expressed in the **global coordinate frame** to formulate constraint equations.

---

### Global Expression of a Body-Fixed Vector

A vector $\boldsymbol{s}_i$ fixed in body $i$, between local points $B$ and $C$, has global components:

$$\boldsymbol{s}_i = \boldsymbol{A}_i (\boldsymbol{s}_i^B - \boldsymbol{s}_i^C)$$

where $\boldsymbol{s}_i^B, \boldsymbol{s}_i^C$: constant local vectors in body $i$, and $\boldsymbol{A}_i$: rotation matrix of body $i$

---

### Vector Connecting Two Bodies

If a vector connects point $B$ on body $j$ to point $B$ on body $i$, the global vector is:

$$\boldsymbol{d} = \boldsymbol{r}_j + \boldsymbol{A}_j \boldsymbol{s}_j^B - \boldsymbol{r}_i - \boldsymbol{A}_i \boldsymbol{s}_i^B$$

---

### Constraint Types

Constraints between such vectors are classified as:

- **Type 1**: between two constant-magnitude vectors
- **Type 2**: between one constant and one variable vector

Notation: $C^{ab}$  
-  $a$: constraint type  
-  $b$: number of scalar constraint equations

These serve as building blocks for defining joints such as:
- Spherical
- Revolute
- Translational
"""
...
To enforce that two vectors remain **perpendicular** (normal) at all times, we impose a single **scalar constraint**: their dot product must be zero.

---

### Type 1 Constraint (Fixed Vectors)

If vectors $\boldsymbol{s}_i$ and $\boldsymbol{s}_j$ are fixed in their respective bodies and must remain perpendicular:

$$\Phi^{(n1,1)} = \boldsymbol{s}_i^T \boldsymbol{s}_j = \boldsymbol{s}_i^T \boldsymbol{A}_i^T \boldsymbol{A}_j \boldsymbol{s}_j' = 0$$

- This is a **normal type 1 constraint** (between two constant-magnitude vectors)
-  $\boldsymbol{s}_i, \boldsymbol{s}_j$: vectors fixed in bodies $i$ and $j$
-  $\boldsymbol{A}_i, \boldsymbol{A}_j$: rotation matrices of bodies $i$ and $j$
-  $\boldsymbol{s}_j'$: local vector in body $j$ coordinates

---

### Type 2 Constraint (One Vector is Variable)

If vector $\boldsymbol{d}$ connects two bodies and must remain perpendicular to a body-fixed vector $\boldsymbol{s}_i$:

$$\Phi^{(n2,1)} = \boldsymbol{s}_i^T \boldsymbol{d} = \boldsymbol{s}_i^T \boldsymbol{A}_i^T (\boldsymbol{r}_j + \boldsymbol{A}_j \boldsymbol{s}_j^B - \boldsymbol{r}_i - \boldsymbol{A}_i \boldsymbol{s}_i^B) = 0$$

-  $\boldsymbol{d}$ is a variable vector between bodies $i$ and $j$
-  $\boldsymbol{s}_i$: fixed in body $i$
- This is a **normal type 2 constraint** (between a fixed and a variable vector)

---

These constraints are used to define **orthogonality** in revolute, cylindrical, or translational joints.
"""

# ╔═╡ 3a048254-d80c-488d-a1f5-25bc7ff5f393
md"""
## Parallel Vector Constraints

To enforce that two vectors remain **parallel**, we use the fact that two vectors are parallel **if and only if** one is perpendicular to two independent directions **orthogonal** to the other.

---

### Parallel-1 Constraint (Fixed Vectors)

To enforce that the vectors $\boldsymbol{h}_i$ and $\boldsymbol{h}_j$ (e.g., $z''$ axes of two bodies) remain **parallel**, we require that:

- $\boldsymbol{h}_j$ is perpendicular to $\boldsymbol{f}_i$ and $\boldsymbol{g}_i$, which are orthogonal to $\boldsymbol{h}_i$

The constraint is:

$$\Phi^{p1}(\boldsymbol{h}_i, \boldsymbol{h}_j) =
\begin{bmatrix}
\Phi^{d1}(\boldsymbol{f}_i, \boldsymbol{h}_j) \\
\Phi^{d1}(\boldsymbol{g}_i, \boldsymbol{h}_j)
\end{bmatrix}
= \begin{bmatrix} 0 \\ 0 \end{bmatrix}$$

- This is called the **parallel-1 constraint**
- Two scalar equations are produced

---

### Parallel-2 Constraint (One Vector is Variable)

If $\boldsymbol{h}_i$ (e.g., a body axis) must remain parallel to a vector $\boldsymbol{d}_{ij}$ (e.g., a displacement vector), then:

$$\Phi^{p2}(\boldsymbol{h}_i, \boldsymbol{d}_{ij}) =
\begin{bmatrix}
\Phi^{d2}(\boldsymbol{f}_i, \boldsymbol{d}_{ij}) \\
\Phi^{d2}(\boldsymbol{g}_i, \boldsymbol{d}_{ij})
\end{bmatrix}
= \begin{bmatrix} 0 \\ 0 \end{bmatrix}$$

- This is the **parallel-2 constraint**
- It becomes invalid if $\boldsymbol{d}_{ij} = \boldsymbol{0}$ (i.e., no direction)
