...
# ╔═╡ c29fbb3f-3d27-46f6-bca8-070c8f1bc11a
begin
...
- `data/linear_targetsTraining.csv`
- `data/linear_inputsTest.csv`
- `data/linear_targetsTest.csv`
...
# ╔═╡ 0d16ae10-b401-4650-a460-c0a843e4b3af
begin
	# Adjust the relative paths as needed:
	df_inputs_train = CSV.read("../../assets/data/linear_inputsTraining.csv", DataFrame; header=false)
	df_targets_train = CSV.read("../../assets/data/linear_targetsTraining.csv", DataFrame; header=false)
	df_inputs_test  = CSV.read("../../assets/data/linear_inputsTest.csv", DataFrame; header=false)
	df_targets_test = CSV.read("../../assets/data/linear_targetsTest.csv", DataFrame; header=false)
	# Convert DataFrames to Float32 matrices
	inputsTraining = Float32.(Matrix(df_inputs_train))
	targetsTraining = Float32.(Matrix(df_targets_train))
	inputsTest  = Float32.(Matrix(df_inputs_test))
	targetsTest = Float32.(Matrix(df_targets_test))
end

# ╔═╡ 05bfd7de-f5f8-4fd8-b4ca-151174cee588
begin
	# Print dataset sizes and dimensions
	nTraining = size(inputsTraining, 1)
	nTest     = size(inputsTest, 1)
	inputDim  = size(inputsTraining, 2)
	outputDim = size(targetsTraining, 2)
	
	println("Training samples: $nTraining, Test samples: $nTest")
	println("Input dimension: $inputDim, Output dimension: $outputDim")
end
...
# ╔═╡ 1e5dd51c-ca5d-4d2e-98b0-e9f628f3b3f7
h = 0.5f0 / 64f0

# ╔═╡ 25fefccc-4925-4881-8f1e-cd074f9cd0cd
t_out = collect(h:h:(h * 64f0))
...
# ╔═╡ 3bf536b3-b017-4fbe-bc39-975c563964c2
# **Define the model: simple linear mapping**

myNeuralNetwork = Chain(
	Dense(inputDim, outputDim, identity; bias=false)
)

# ╔═╡ d8c2df51-a669-40c7-b8ca-0710b84995d2
# **Compute untrained output (for comparison, on first test sample)**
outTest_init = myNeuralNetwork(inputsTest')

# ╔═╡ e9df1dad-c49a-4094-a654-4b838bdddce3
untrained_weights = copy(myNeuralNetwork[1].weight)

# ╔═╡ e0e0a368-837f-4fdc-9535-d8323f4e3753
# visualize linear Layer. In the untrained state this is "just" noise and depends only on the initialization procedure.
heatmap(
untrained_weights,  # (outputDim × inputDim)
xlabel = "Input Index",
ylabel = "Output Index",
title  = "Untrained Weight Matrix (Heatmap)"
)
...
* optimizer choice and settings: here ADAM is applied with learning rate 5e-2 (which is very high for most examples).
...
begin
	# Transpose to shape (features, samples) for Flux
	
	X_train = inputsTraining'   # (inputDim, nTraining)
	Y_train = targetsTraining'  # (outputDim, nTraining)
	X_test  = inputsTest'       # (inputDim, nTest)
	Y_test  = targetsTest'      # (outputDim, nTest)
	
	# Create DataLoaders
	train_loader = DataLoader((X_train, Y_train), batchsize=batchSize, shuffle=true)
	val_loader   = DataLoader((X_test, Y_test),   batchsize=batchSize, shuffle=false)
...
begin
	...
	for epoch in 1:nEpoch
	    ...
	    for (x_batch, y_batch) in train_loader
	        ∇model = gradient(m -> loss_fn(m(x_batch), y_batch), myNeuralNetwork)[1]
	        Optimisers.update!(opt_state, myNeuralNetwork, ∇model)
	        running_loss += loss_fn(myNeuralNetwork(x_batch), y_batch)
	    end
	    push!(loss_epoch, running_loss / length(train_loader))
	
	    # Validation every 10 epochs
	    if epoch % 10 == 0
	        push!(lossEpochSteps, epoch)
	        total_val_loss = 0f0
	        for (x_val, y_val) in val_loader
	            total_val_loss += loss_fn(myNeuralNetwork(x_val), y_val)
	        end
	        push!(lossValidation, total_val_loss / length(val_loader))
	    end
	end
	println("Training completed in $(round(time() - t_start, digits=2)) seconds")
end
...
# ╔═╡ 025daf57-3f41-4d36-b5dc-dd02ac5653f3
heatmap(
myNeuralNetwork[1].weight,  # (outputDim × inputDim)
xlabel = "Input Index",
ylabel = "Output Index",
title  = "Trained Weight Matrix (Heatmap)"
)
...
xlabel="Time",
ylabel="Weight Value",
# title="Weights of First Output Unit",
labels=[L"displacement due to $x_0$" L"displacement due to $\dot{x}_0$"]
)
...
# ╔═╡ 09f48295-6ebe-4266-8ac0-3248f0cb4917
begin
	outTest_all  = myNeuralNetwork(X_test)  # (outputDim, nTest)
	pred_sample1 = outTest_all[:, 1]        # first test sample
	true_sample1 = Y_test[:, 1]             # true target for first test sample

	plt5 = plot(
	    t_out,
	    outTest_init[:, 1],
	    label = "Untrained Network (Sample 1)",
	    linestyle = :solid
	)
	plot!(
	    t_out,
	    pred_sample1,
	    label = "NN Prediction (Sample 1)",
	    linestyle = :solid
	)
	plot!(
	    plt5,
	    t_out,
	    true_sample1,
	    label = "True Signal (Sample 1)",
	    linestyle = :dash
	)
	xlabel!("Time (s)")
	ylabel!("Displacement")
	title!("Prediction vs. True Signal (Test Sample 1)")
end
