...
- How to find a solution to an equation in the general form 
$f(x)=0$
- For example, $e^{-x} \sin x = \cos x$, thus $f(x) = e^{-x} \sin x - \cos x$. 
- We want to find an approximate solution rather than an exact one. This problem is also called root finding. Main areas:
...
- Based on the collection of points $(x, f(x))$ – used e.g., in plotting. 
- Simply go through all points and check when function crosses axis. 
- This requires a lot of computation; therefore, the name: _brute force_. 
...
begin
    ...
    """
    brute_force_method_idea() → Plot

Plot a smooth “true” curve u_fun(t) in black, show sampled points
at t_min:Δt:t_max with a blue dashed line and circle markers, and
draw a red dashed horizontal line at y=0.
    """
    function brute_force_method_idea()
		
		# 1) build the data
		x = range(0, 4, length=51)
		y = exp.(-x) .* cos.(4 .* x)
		
		# 2) first plot the smooth line
		plt = plot(
		    x, y;
		    linewidth = 1.5,
		    color     = :blue,
		...
		# 3) overlay every‐other marker in black
		scatter!(
		    plt,
		    x[1:2:end], y[1:2:end];
		    marker            = :circle,
    ...
    # Demo: call with default test function and mesh
    brute_force_method_idea()
end
...
- We have a set of points $(x_i, y_i)$, $y_i = f(x_i)$ $i = 0, \ldots, n$, and $x_0 < \ldots < x_n$. 
- Curve crosses $x$ axis when $y_i$ and $y_{i+1}$ have different signs: $y_i y_{i+1} \leq 0$. That is $f(x) = 0$ is in $[x_i, x_{i+1}]$. 
- Assuming linear variation in the interval, the root is located at 
$x = x_i - \frac{(x_{i+1} - x_i)}{(y_{i+1} - y_i)} y_i$. 
- Now, take a look at the implementation.
- In the MATLAB course, I am discussing preallocation at this point. But Julia's implementation handles this out of the box.
"""

# ╔═╡ 5354a952-28f3-4ce0-bc93-ec6df1e25eee
"""
    brute_force_root_finder(f, a, b, n)

Approximate all roots of a scalar function `f` in the interval `[a,b]` by
sampling `n` points and using linear interpolation on each subinterval
where a sign change occurs.

# Arguments
- `f` : a function `f(x)` (for scalar `x`) returning a `Number`.
- `a, b` : end‐points of the search interval.
- `n` : number of equally‐spaced sample points.

# Returns
- `roots` : a `Vector{Float64}` of approximate zero crossings.
"""
function brute_force_root_finder(f, a::Float64, b::Float64, n::Int)
    x = range(a, b; length=n)           # sample points
    y = f.(x)                           # evaluate f at each x
    roots = Float64[]                   # collect zero‐crossings

    for i in 1:(n-1)
        # check for sign change or exact zero
        if y[i] * y[i+1] ≤ 0
            # linear interpolation: solve y=0 on [x[i], x[i+1]]
            Δx = x[i+1] - x[i]
            Δy = y[i+1] - y[i]
            root = x[i] - (Δx/Δy) * y[i]
            push!(roots, root)
...
# ╔═╡ 7ae92599-9c2d-4cda-90f0-f519a74d7c23
begin
	md"""
	...
	function demo_brute_force_root_finder()
	    # 1) Define the test function and sampling parameters
	    ...
	    # 5) Overlay roots (if any) at y=0
	    if !isempty(roots)
			println(roots)
	        scatter!(
	            plt,
	            roots, zeros(length(roots));
	            color      = :red,
	            marker     = :circle,
	            markersize = 6,
	            label      = L"roots"
	    ...
	    # **Return the plot object** so Pluto will display it
	    plt
	end
...
- Famous and widely used in practice. 
- The NR method is **fast** but does _not guarantee_ that a solution will be found. 
- Idea: construct a series of linear equations (as we know how to solve them) and expect that they will bring us to the solution of a nonlinear one. 
- We will test function $f(x) = x^2 - 9$ on interval $[0, 1000]$.
"""

# ╔═╡ c30173a8-725d-49ff-87b3-30e360cf87ed
### Pluto.jl cell ###
begin
    md"""
    plot_quadratic_zero(; x_min, x_max, n) → Plot

Plot \(f(x)=x^2-9\) over \([x_{\min},x_{\max}]\) with a blue curve
and a dashed red horizontal line at \(y=0\).
    """
    function plot_quadratic_zero(; x_min::Float64=0.0,
                                  x_max::Float64=10.0,
                                  n::Int=400)
        # 1) sample points
...
- Next, we approximate the function at this point with a tangent line 
$\tilde{f}(x) = f(x_0) + f'(x_0)(x - x_0)$
- Tangent slope is $f'(x_0)$ and it touches $f(x)$ curve at $x_0$. 
- Tangent crosses the x-axis at 
$x = x_0 - \frac{f(x_0)}{f'(x_0)}$
- This is a new candidate point $x_1$.
- Starting with $x_0$ iterate 
$x_{n+1} = x_n - \frac{f(x_n)}{f'(x_n)}$
- For $n = 0, 1, 2, \ldots$. 
- Until $|f(x_n)| < \varepsilon$ and $\varepsilon$ is a small number. 
- Look at implementation – we are not using an array for $x_n$.
"""

# ╔═╡ c5ab7767-0dcd-4c8a-b37b-4d89c7f0b207
"""
    naive_newton(f, dfdx, x0, tol) → x

Perform Newton’s method starting from `x0` until `|f(x)| ≤ tol`.
Prints each iterate and returns the root approximation.
"""
function naive_newton(f::Function, dfdx::Function, x0::Float64, tol::Float64)
    x = x0
    while abs(f(x)) > tol
        x = x - f(x) / dfdx(x)
        println("iterate: ", x)
...
begin
	...
	xr = naive_newton(f, dfdx, 1000.0, 1e-6)
	println(xr)
end
...
git-tree-sha1 = "0492be06770b3609a9729b79b42bde329b5f617b"
